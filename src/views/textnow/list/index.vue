...
<!--            </a-button>-->
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'status'">
            ...
            <a-tag v-else-if="record.status===3 && record.login_message !== 'success'" color="red">{{record.login_message}}</a-tag>
    ...
    <textnow-import v-model:visible="showImport" :groupId="groupId" @done="reload" />
    <textnow-add v-model:visible="showEdit" :data="current" :isAdmin="adminRef" @done="reload" />
  </div>
</template>
<script setup>
...
import TextnowAdd from "@/views/textnow/list/textnow-add.vue";
import {UploadOutlined,PlusOutlined} from '@ant-design/icons-vue';
import TextnowImport from "@/views/textnow/list/textnow-import.vue";
import { GetTextnowList,DelTextnowAccount } from '@/api/admin/textnow'
...
const remove = (row)=>{
  DelTextnowAccount({ids:[row.id]}).then((msg)=>{
    message.success(msg)
    reload()
...
const datasource = ({ page, limit, where, orders }) =>{
  return GetTextnowList({ ...where, ...orders, page, limit,id:groupId.value })
}
// 表单数据
