...
<script setup>
  ...
  const query = () => {
    ...
    listRoleMenus(props.data.roleId).then((data) => {
        authLoading.value = false;
        // 转成树形结构的数据
        authData.value = toTreeData({
          data: data?.map((d) => ({...d, key: d.menuId, icon: undefined, menuIcon: d.icon})),
          idField: 'menuId',
  ...
  const save = () => {
    ...
    updateRoleMenus(props.data?.roleId, Array.from(ids)).then((msg) => {
        loading.value = false;
        message.success(msg);
        updateVisible(false);
    }).catch((e) => {
