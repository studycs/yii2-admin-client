...
export async function addUser(data) {
  const res = await request.post('/system/add-user', data);
  if (res.data.code === 0) {
...
export async function updateUser(data) {
  const res = await request.put('/system/update-user', data);
  if (res.data.code === 0) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
...
export async function removeUser(id) {
  const res = await request.get('/system/del-user',{params:{id}});
  if (res.data.code === 0) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
...
export async function updateUserPassword(userId, password = '123456') {
  const res = await request.post('/system/reset-password', { userId, password });
  if (res.data.code === 0) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
...
export async function checkExistence(field, value, id) {
  const res = await request.get('/auth/userinfo', {params: { field, value, id }});
  if (res.data.code === 0) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
